...
def main():
    ...
    protocol=dict(version=2,corpus='data_no_punct_v1',train_characters=len(train),vocabulary_characters=len(vocabulary),
                  ...
                  source_sha256={str(p.relative_to(HERE)):hashlib.sha256(p.read_bytes()).hexdigest() for p in
                      [Path(__file__),HERE/'hebb_text.py',HERE/'run_experiment.py',data/'train.txt',data/'validation.txt',data/'test.txt',
                       ...
                       HERE/'results_no_punct_v1/adjacent_diminishing/model.npz',
                       HERE/'run_short_context.py',HERE/'short_context_v1/failure_diagnostic.json']})
    ...
    for name in names:
        ...
        # Effective activity is identical after 64 chars. Dictionary insertion
        # order can survive longer through repeated characters, so float32 sums
        # may differ slightly. Check decisions exactly, probabilities to tolerance.
        deltas=[]
        for a,c in zip(scored['rows'],interventions['full']['rows']):
            assert a['position']==c['position'] and a['rank']==c['rank'] and a['predicted']==c['predicted']
            assert a['known']==c['known']
            if a['known']:
                assert math.isclose(a['probability'],c['probability'],rel_tol=1e-7,abs_tol=1e-12)
                deltas.append(abs(a['probability']-c['probability']))
        result['continuous_vs_prefix_check']=dict(rank_and_prediction_exact=True,
            probability_relative_tolerance=1e-7,probability_absolute_tolerance=1e-12,
            max_absolute_probability_difference=max(deltas,default=0.))
